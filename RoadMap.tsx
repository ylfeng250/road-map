...
const RoadMap: React.FC = () => {
  ...
  // 处理筛选条件变化
  const handleFilterChange = (city: string, area: string, distanceRange: string, ascentRange: string) => {
  ...
  const filteredRoutes = useMemo(() => {
    return routes.filter(route => {
      ...
      if (selectedDistanceRange) {
        ...
        if (selectedDistanceRange === 'medium' && (distance < 7 || distance > 9)) {
          return false;
        }
      ...
      if (selectedAscentRange) {
        ...
        if (selectedAscentRange === 'medium' && (ascent < 400 || ascent > 600)) {
          return false;
        }
  ...
  }, [routes, selectedCity, selectedArea, selectedDistanceRange, selectedAscentRange]);

  // 加载状态
  if (loading) {
    return (
      <Layout title="户外徒步路线地图" subtitle="探索杭州及周边地区的精选徒步路线">
        <div style={{ textAlign: 'center', padding: spacing.xxl }}>
          <div style={{ fontSize: '24px', marginBottom: spacing.md }}>加载中...</div>
          <div style={{
            width: '50px',
            height: '50px',
